...
import (
	...
	"github.com/lucap9056/magic-conch-shell/core/v2/internal/grpcserver"
...
func runConsoleMode(asst *assistant.Client, life *lifecycle.LifecycleManager) {
	...
	language, _ := languages.Parse(os.Getenv("LANG"))
	...
	for {
		...
		case input, ok := <-inputChan:
			...
			func(text string) {
				...
				res, err := asst.GenerateResponse(reqCtx, language, message, nil)
				if err != nil {
...
func assistantTest(llm *assistant.Client) (string, error) {
	...
	reply, err := llm.GenerateResponse(ctx, languages.EN, message, nil)
	if err != nil {
