...
import (
	...
	"github.com/lucap9056/magic-conch-shell/core/v2/assistant/luascript"
...
//go:embed systemInstruction
var systemInstructionFS embed.FS
...
type Client struct {
	...
	modelName          string
	systemInstructions map[languages.Language]string
	luaRuntimeScript   string
}
...
func NewClient(apiKey string, modelName string, allowedImageDomains string) (*Client, error) {
	...
	sysInstructions := make(map[languages.Language]string)
	supportedLanguages := []languages.Language{languages.EN, languages.ZH}
	for _, lang := range supportedLanguages {
		fileName := fmt.Sprintf("systemInstruction/%s.txt", lang)
		data, err := systemInstructionFS.ReadFile(fileName)
		if err != nil {
			return nil, fmt.Errorf("failed to load system instruction for language %s: %w", lang, err)
		}
		sysInstructions[lang] = string(data)
	}
	...
	return &Client{
		...
		luaRuntimeScript:   luascript.GetScript(),
	}, nil
}

// GenerateResponse sends a message (and optional images) to the LLM and processes the Lua-formatted output.
func (c *Client) GenerateResponse(ctx context.Context, language languages.Language, newMessage *structs.PromptMessage, historyMessages []*structs.PromptMessage) (string, error) {

	model := c.genaiClient.GenerativeModel(c.modelName)

	model.SystemInstruction = &genai.Content{
		Role:  "system",
		Parts: []genai.Part{genai.Text(c.systemInstructions[language])},
	}
...
func (c *Client) safeExecuteLua(luaCode string) (string, error) {
	...
	if err := L.DoString(c.luaRuntimeScript); err != nil {
